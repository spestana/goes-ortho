...
def test_goes_clip_conus(setup_session):
    ...
    bounds = [-110, 30, -100, 40]  # [min_lon, min_lat, max_lon, max_lat]
    newfilepath = path.abspath("./tmp.nc")
    # run goes clip function
    go.clip.subsetNetCDF(filepath, bounds, newfilepath)
    # do we have a dataset?
    assert isinstance(
        xr.open_dataset("tmp.nc"), xr.core.dataset.Dataset
    ), "unable to open file"
    ds = xr.open_dataset("tmp.nc")
    # does it have non-zero dimensions?
    assert (
        ds.dims["y"] != 0
    ), "y dimension should be non-zero, check the latitude bounds you've provided"
    assert (
        ds.dims["x"] != 0
    ), "x dimension should be non-zero, check the longitude bounds you've provided"
    ds.close()
