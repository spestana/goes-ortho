...
def test_LonLat2ABIangle(setup_session):
    ...
    filepaths = glob.glob(
        "./tests/resources/spestana-goes-ortho-data-*/data/C/OR_ABI-L2-ACHAC-M6_G17_s20221210056177_e20221210058550_c20221210101260.nc"
    )
    ...
    assert (
        x == 0.06993880298923384
    ), "ABI Fixed Grid x coordinate (radians) should be a numpy.float64"
    assert (
        y == 0.11588291952114481
    ), "ABI Fixed Grid y coordinate (radians) should be a numpy.float64"


def test_both(setup_session):
    # get geometry metadata from one of our Full Disk test/example datasets
    filepaths = glob.glob(
        "./tests/resources/spestana-goes-ortho-data-*/data/F/OR_ABI-L1b-RadF-M6C16_G18_s20230370050213_e20230370059532_c20230370059568.nc"
    )
    print(filepaths[0])
    with xr.open_dataset(filepaths[0]) as ds:
        req = ds.goes_imager_projection.semi_major_axis
    ...
    original_lat = -45
    original_lon = 170
    (x, y) = go.geometry.LonLat2ABIangle(
        original_lon, original_lat, 0, H, req, rpol, e, lon_0
    )
    ...
    assert (
        abs(lon - original_lon) < 1e-9
    ), "Longitude does not match original longitude after conversions"
    assert (
        abs(lat - original_lat) < 1e-9
    ), "Latitude does not match original latitude after conversions"
