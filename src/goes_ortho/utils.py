            ...
            while k <= len(day) - 1:
                julian = str(
                    datetime.datetime.strptime(
                        "{0}-{1}-{2}".format(year[i], month[j], day[k]), "%Y-%m-%d"
                    )
                    .timetuple()
                    .tm_yday
                ).zfill(3)
...
def __isAList(year, month, day, product, hour, channel=None, julianDay=None):
    ...
    to be use in the for loops: If all the arguments are List of string, the function will not convert to List.
...
class ProgressPercentage(object):
    ...
    def __call__(self, bytes_amount):
        ...
        with self._lock:
            ...
            sys.stdout.write(
                "\r{0}: [{1}] {2}%  {3} MB/{4} MB".format(
                    self._filename,
                    "#" * int((percentage / 10)),
                    int(percentage),
                    self._seen_so_far / 1e6,
                    self._size / 1e6,
                )
            )
...
def bannerDisplay(version):
    ...
    print(banner)
    print(
        " A Python package can be useful to download dataset from goes satellite on AWS  %sv"
        % version
    )
    print(
        "=========================================================================================="
